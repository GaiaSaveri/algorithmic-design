 ...
 */
float** fix(float** C, const size_t r_b, const size_t c_b, const size_t r_a)
{
  float **CC = allocate_matrix(r_b, c_b);
 ...
 * function returns.
 */
float** lego_bricks_multiplication(float const *const *const A,
                                   const size_t A_r, const size_t A_c, //dimensions of A
                                   float const *const *const B,
                                   ...
                                   )
{
  ...
  float** C = allocate_matrix(n,n); //this will be returned
  ...
  for(size_t i=0; i<n_blocks; i++)
  {
    ...
    sum_matrix(C,(float const *const *const)C,(float const *const *const)CC,n);
  }
 ...
 */
float** squared_matrix_padding(float const *const *const A,
                               const size_t A_r, const size_t A_c,  //dimensions of A
                               float const *const *const B,
                               const size_t B_r, const size_t B_c //dimensions of B
                             )
{
    ...
    {
      float** C = allocate_matrix(A_r, A_r);
      strassen_matrix_multiplication(C, A, B, A_r);
      return C;
    }
    else //the dimension is not a power of two
    {
      ...
      float **C = allocate_matrix(p,p);
      strassen_matrix_multiplication(C, (float const *const *const)Pad_A, (float const *const *const)Pad_B, p);
      deallocate_matrix(Pad_A, p);
      deallocate_matrix(Pad_B, p);
      float** CC = fix(C, A_r, A_c, p);
      return CC;
    }
}
                        ...
                        )
{
  if(A_r==A_c) //A is a squared matrix
  {
    ...
    {
      float** C = squared_matrix_padding(A, A_r, A_c, B, B_r, B_c);
      return C;
    }
    else //B is not squared
    {
      if(A_r>B_c)
      {
        ...
        float** C = squared_matrix_padding(A, A_r, A_c, (float const *const *const)B_sq, p, p);
      ...
      else //B_c>A_r
      {
        ...
        float** B_sq = padding_submatrix(B, 0, 0, B_r, B_c, p);
        float** C = squared_matrix_padding((float const *const *const)A_sq, p, p, (float const *const *const)B_sq, p, p); //or directly Strassen
  ...
  else //A is not squared
  {
    ...
    {
      float** C = lego_bricks_multiplication(A, A_r, A_c, B, B_r, B_c);
      return C;
    }
  }
}
