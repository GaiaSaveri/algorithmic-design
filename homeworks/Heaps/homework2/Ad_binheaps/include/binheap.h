 ...
 */
typedef struct {
  void *A; //this is the array used to store the heap nodes' keys
  //it holds: A[i] =  A[pos_key[rev_key[i]]]
  unsigned int *pos_key; //position of the key of a node inside A <--
  unsigned int *rev_key; //node (in pos_key) corresponding to a given position (in A) <--
  unsigned int num_of_elem; //this is the number of nodes currently in the heap
  ...
  void *max_order_value; //this is the maximum value ever stored in the heap
} binheap_type;
